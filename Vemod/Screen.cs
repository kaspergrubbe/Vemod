...
namespace Vemod
{
    public static class Screen
    {
         ...
         * minMatch = minimum match in percent to add a match to the list (default=0.99)
         */
        public static PixelMatchList SearchForSamples(Color[,] screen, Color[,] sample, Square offset, float minMatch)
        {
            ...
            while (i < offset.endx)
            {
                while (j < offset.endy)
                {
                    ...
                    if (Vemod.Image.rgbPercentCompare(screen[i, j], sample[k, l]) > 0.70)
                    {
                        // Pixelmatches:
                        int pixelmatches = 0;
        ...
        {
            return Vemod.Screen.SearchForSamples(screen, sample, new Vemod.Square(0, 0, screen.GetLength(0), screen.GetLength(1)), (float)0.995);
        }
         ...
         * minMatch = minimum match in percent to add a match to the list (default=0.99)
         */
        public static PixelMatchList SearchForSamples2(Color[,] screen, Color[,] sample, Square offset, float minMatch)
        {
            PixelMatchList matches = new PixelMatchList();
            ...
            sample = Image.shrinkImage(sample);
            // count sample rows
            int sample_rows = sample.GetLength(0);
            int sample_colums = sample.GetLength(1);
            ...
            while (x < sample_colums)
            {
                while (y < sample_rows)
                {
                    if (sample[x, y].A < 200)
                    {
                        pixels.Add(new Pixel(x, y, sample[x,y]));
                    }
                    y++;
                }
                y = 0;
